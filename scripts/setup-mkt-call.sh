...
mkdir -p "$ROOT_DIR/front/mkt.smarterbot.cl" "$ROOT_DIR/services/call.smarterbot.cl" "$ROOT_DIR/front/fulldaygo.smarterbot.cl"
...
if [ -z "$(ls -A "$ROOT_DIR/front/fulldaygo.smarterbot.cl" 2>/dev/null || true)" ]; then
  git clone https://github.com/SmarterCL/fulldaygo.smarterbot.cl "$ROOT_DIR/front/fulldaygo.smarterbot.cl"
else
  echo "- Ya existe contenido en front/fulldaygo.smarterbot.cl (omitido)"
fi
...
echo "- https://fulldaygo.smarterbot.cl"
